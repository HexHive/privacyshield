        ...
        switch (state) {
            case init:
                if (isspace((unsigned char)*cp)) {
                    ...
                    json_debug_trace(
                        (1, "Non-WS when expecting object start.\n"));
                    if (end != NULL) {
                        *end = cp;
                    }
            ...
            case in_escape:
                ...
                switch (*cp) {
                    ...
                    case 'u':
                        cp++; /* skip the 'u' */
                        for (n = 0; n < 4 && isxdigit((unsigned char)*cp);
                             n++) {
                            uescape[n] = *cp++;
                        }
                        ...
                        *pval++ =
                            (unsigned char)u; /* truncate values above 0xff */
                        break;
            ...
            case post_val:
                ...
                if (cursor->map != 0) {
                    ...
                    json_debug_trace(
                        (1, "Invalid enumerated value string \"%s\".\n",
                         valbuf));
